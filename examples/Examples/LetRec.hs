  ...
  testGroup
    ...
    , testGroup
      ...
      , testCase "nested record access term"
        $ printTerm (pmatch' (rcon rawFlatOuter) $
      ...
      , testCase "nested match term"
        $ printTerm (rmatch (rcon rawFlatOuter) $ \(FlatOuterRecord {..}) ->
      ...
      , testCase "nested match value"
        $ equal' (rmatch (rcon rawFlatOuter) $ \(FlatOuterRecord {..}) ->
                        rmatch (rcon flatInner2) $ \(SampleRecord {..}) ->
                        sampleString)
    ...
    , testGroup
      ...
      , testCase "nested match term"
        $ printTerm (pmatch' sampleShallowOuter $ \(PRecord ShallowOuterRecord {..}) ->
      ...
      , testCase "nested match value"
        $ equal' (pmatch' sampleShallowOuter $ \(PRecord ShallowOuterRecord {..}) ->
                     pmatch shallowInner2 $ \(PRecord SampleRecord {..}) ->
                     sampleString)
          "(program 1.0.0 \"Salut, Monde!\")"
      ]
