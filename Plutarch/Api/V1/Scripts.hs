...
import Plutarch.Builtin (punsafeFromData, punsafeAsData)
import Plutarch (ClosedTerm, POpaque, popaque, compile)
import Plutarch.Prelude
import Plutarch.Api.Internal.Scripts 
  (hashScriptWithPrefix, serialiseScript, datumHash)
...
-}
pwrapMintingPolicyAsData :: 
  (forall s. Term s (PAsData redeemer) 
    -> Term s (PAsData ctx) 
    -> Term s p)
