...
import Plutarch.Internal.Term (RawTerm (RCase, RConstr), S, Term (Term), TermResult (TermResult), asRawTerm, getDeps, getTerm, phoistAcyclic, punsafeCoerce, (#), (:-->))

import Plutarch.Repr.Internal (PRec (PRec, unPRec), PStruct (PStruct, unPStruct), RecTypePrettyError, StructSameRepr, UnTermRec, UnTermStruct, grecEq, gstructEq, pletL)
...
newtype DeriveAsSOPStruct (a :: S -> Type) s = DeriveAsSOPStruct {unDeriveAsSOPStruct :: a s}
...
newtype DeriveAsSOPRec (a :: S -> Type) s = DeriveAsSOPRec {unDeriveAsSOPRec :: a s}
...
instance
  ...
  pmatch' x f =
    pmatch x (f . DeriveAsSOPRec . to . hcoerce . SOP . (Z @_ @_ @'[]) . unPRec . unPSOPRec)
