...
module Plutarch.Repr.Tag where
...
newtype PTag (struct :: [S -> Type]) (s :: S) = PTag {unPTag :: Term s PInteger}
  deriving stock (GHC.Generic)

instance SOP.Generic (PTag struct s)
deriving via DeriveAsNewtype (PTag struct) instance PlutusType (PTag struct)

newtype DeriveAsTag (a :: S -> Type) s = DeriveAsTag {unDeriveAsTag :: a s}
...
instance (forall s. TagTypeConstraints s a struct) => PlutusType (DeriveAsTag a) where
  ...
  pcon' (DeriveAsTag x) =
    pconstant @PInteger $ toInteger $ hindex $ from x
  ...
  pmatch' tag f = unTermCont $ do
    ...
    let
      ...
      handlers' = cpara_SList (Proxy @IsEmpty) (TagMatchHandler $ \_ _ -> Nil) go

      handlers :: [(Integer, Term s b)]
      handlers = hcollapse $ unTagMatchHandler handlers' 0 g

    pure $ groupHandlers handlers tag'

newtype TagLiftHelper r struct = TagLiftHelper {unTagLiftHelper :: Integer -> (SOP I struct -> r) -> r}
