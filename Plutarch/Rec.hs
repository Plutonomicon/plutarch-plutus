...
import Data.Monoid (Endo(Endo, appEndo), Sum(Sum, getSum), Dual(Dual, getDual))
import Numeric.Natural (Natural)
import Plutarch (PCon, pcon, phoistAcyclic, plam, punsafeCoerce, (:-->), (#))
import Plutarch.Internal (RawTerm(..), Term(Term, asRawTerm), TermResult(..), mapTerm)
...
{- |
  ...
  >>> record #. field
-}
(#.) :: forall r s t. (Rank2.Distributive r, Rank2.Traversable r)
      => Term s (PRecord r) -> (r (ScottArgument r s) -> ScottArgument r s t) -> Term s t
...
rcon r = plam (\f-> punsafeCoerce $ appEndo (getDual $ Rank2.foldMap (Dual . Endo . applyField) r) f)
  where applyField x f = punsafeCoerce f # x
...
accessors = Rank2.cotraverse accessor id
  where accessor :: (r (ScottArgument r s) -> ScottArgument r s a) -> ScottArgument r s a
        ...
        next _ = do {i <- get;
                     let {i' = pred i};
                     seq i' (put i');
                     return (ScottArgument $ phoistAcyclic $ Term $ const $ TermResult{getTerm= RLamAbs (fieldCount - 1) $ RVar i',
                                                                                        getDeps= []})}
        fieldCount :: Natural
