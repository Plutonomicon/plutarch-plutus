...
- `phasZeroTokenQuantities`, `phasAdaEntry`, and `phasZeroAdaEntry` functions to
...
- Lookup-related functions now operate only on **sorted** assoc maps.
  - Affected functions: `plookup`, `plookupData`, `plookupDataWith`,
    `pfindWithDefault`, `pfoldAt`, `pfoldAtData`, `ptryLookup`, and `pvalueOf`.
...
- In `Plutarch.LedgerApi.Value`, `passertSorted` has been deprecated in favor of
  `ppromoteToSortedValue`.
- `PSortedMap` and all sorted Value types now disallow duplicate keys.
- `pfindInputByOutRef` has been renamed to `pfindTxInByTxOutRef`, its signature
  updated to match the corresponding `plutus-ledger-api` definition, and the
  function has been moved to `Plutarch.LedgerApi.V3.Contexts`.
...
- Newtypes representing mint Values: `V1.MintValue` and `V3.MintValue`, along
  with their construction functions: `pempty`, `psingleton`, and `ptoMintValue`
...
- All ledger types are now derived with new derivation tactics--namely ones
  found in `Plutarch.Repr.Data`.
...
- Bug in `PNum` instance for `PPosixTime`, where `#*` was implemented as `#+
  instead
- `pcontains` bug where open intervals would not be handled correctly (see
  [this issue](https://github.com/Plutonomicon/plutarch-plutus/issues/705))
...
- `plutarch-api` is renamed to `plutarch-ledger-api`, and any modules
  `Plutarch.Api.*` is renamed to `Plutarch.LedgerApi.*`
...
- `padaSymbol`, `padaToken`, `plookup`, `plookupData`, `plookupDataWith`,
  ...
  `pfoldlWithKey`, `pkeys`, `pkvPairKey`, `pkvPairValue`, `pkvPairLt` to
...
- `pparseDatum` now takes `PMap` as datum lookup table in order to be compatible
  with Ledger V2 types.
