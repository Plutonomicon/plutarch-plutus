...
Any use of `x` will inline the **full definition** of `x`. `x + x` will duplicate `something complex` in the AST. To avoid this, you should [use `plet` in order to avoid duplicate work](./TRICKS.md#dont-duplicate-work). Do note that this is **strictly evaluated, and hence isn't always the best solution.**
