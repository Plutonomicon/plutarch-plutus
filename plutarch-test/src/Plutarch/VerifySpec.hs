...
import Plutarch.Verify ( PTryFrom (ptryFrom)
...
checkShallow :: forall (target :: PType) (actual :: PType). 
  (PTryFrom target, PIsData actual) => ClosedTerm actual -> ClosedTerm target
checkShallow = reprTargetActual ptryFrom

checkDeep :: forall (target :: PType) (actual :: PType). 
  (PTryFromRecur target, PIsData actual) => ClosedTerm actual -> ClosedTerm target
checkDeep = reprTargetActual ptryFromRecur

reprTargetActual :: forall (target :: PType) (actual :: PType). 
  (PIsData actual ) => ClosedTerm (PData :--> target) -> ClosedTerm actual -> ClosedTerm target
